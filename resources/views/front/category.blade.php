...
<body class="font-[Poppins] pb-[83px] px-4 sm:px-6 lg:px-0">
	...
	<section id="Category-result" class="max-w-[1130px] mx-auto flex items-center flex-col gap-5 sm:gap-[30px] mt-10 sm:mt-[50px] md:mt-[70px]">
		...
		<div id="search-cards" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-[30px] w-full">
			...
			<a href="{{route('front.details', $news->slug)}}" class="card">
				<div
					...
					<div class="thumbnail-container h-[150px] sm:h-[180px] md:h-[200px] relative rounded-[20px] overflow-hidden">
						<div
					...
					<div class="flex flex-col gap-[6px]">
						<h3 class="text-base sm:text-lg leading-tight sm:leading-[27px] font-bold">{{ substr($news->name, 0, 70)}}{{ strlen($news->name)> 70 ? '...':''}}</h3>
						<p class="text-xs sm:text-sm leading-[21px] text-[#A3A6AE]">{{$news->created_at->format('d M, Y')}}</p>
					</div>
				</div>
			</a>
            @empty
	...
	<section id="Advertisement" class="max-w-[1130px] mx-auto flex justify-center mt-10 sm:mt-[50px] md:mt-[70px]">
		<div class="flex flex-col gap-3 w-full">
			...
			<p class="font-medium text-xs sm:text-sm leading-[21px] text-[#A3A6AE] flex gap-1 justify-center sm:justify-start">
